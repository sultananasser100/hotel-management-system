...
## 2026-09-23 — Production prep: generate Prisma Client on install; `migrate deploy` for production

The generated client (`src/generated/prisma`) is gitignored, and nothing regenerated it on a fresh
install, so a clean checkout (e.g. on Vercel) couldn't build. Added `"postinstall": "prisma
generate"` rather than changing the `build` script: it covers every install, local or hosted, and
leaves `build` as plain `next build`. Verified with a clean `npm ci` + `npm run build`; `prisma
generate` also works with no `DATABASE_URL` set. Production migrations use a separate
`db:deploy` (`prisma migrate deploy`, applies committed migrations only) — `db:migrate`
(`prisma migrate dev`) stays development-only, since it can create migrations and reset the
database.

Also in this cleanup: the assistant's code default for `GEMINI_MODEL` was changed from
`gemini-3.7-flash` to `gemini-3.5-flash` to match `.env.example` and the model already in use
(3.7 returned frequent 503s on the free tier). Deployment docs went into the README's Deployment
section rather than a new file, to keep one place for setup instructions.
